...
在达菲网页里直接使用洛雪订阅源：**导入订阅（在线 URL / 粘贴 / 文件 / 目录）→ 榜单与歌单浏览 →
聚合搜索 → 多音质选择 → 跨源自动降级 → 播放与拖动进度条**。

> **本仓库不附带任何订阅源脚本，也不指名任何音乐平台**：可用的平台完全取决于你自己导入的订阅源
> （插件只按订阅源声明的平台能力工作）。仅供个人学习研究使用。
...
- **订阅源管理**：在线 URL / 粘贴 / 本地文件 / 目录导入；逐源启停、排序、重新拉取、删除；
  同内容自动判重；源正文落持久目录，升级插件不丢
- **浏览**：订阅源所声明平台的**榜单**（可翻页）、**歌单发现**（推荐 / 最热 / 最新）、
  **聚合搜索**（歌曲 + 歌单）、整个榜单 / 整张歌单入队播放
- **音质**：多档位（含无损与高解析档位），取链后做**可播性探测**（range GET + 音频魔数嗅探）并自动降级
- **播放链路**：
  - 常驻解析进程（取链 + 校验各一）与解析缓存、预取，起播更快
  - 用**自己嗅探出的真实格式**覆盖 CDN 谎报的 Content-Type（有的平台把无损文件声明成别的容器，
    不修就是"进度条在走但没声音"）
  - **https 直链**走 LMS 自带的 HTTPS 处理器（派生自 `Slim::Player::Protocols::HTTPS`），
    只有 https 直链的平台因此才能播放
  - **拖动进度条可用**：码率/时长同时发布到伪 URL 与真实直链 URL，并自实现 `getSeekData`
- **封面**：队列/正在播放封面（含需要官方接口或插件代理才能取到真图的平台）
- **插件图标**：插件管理器与「应用」菜单显示洛雪官方图标

## 版本

当前 **v0.11.20**（2026-09-21）。v0.5.9 之后的一整条 0.6.0 ~ 0.11.20 线此前只在本机局域网仓库发布，
本 release 是首次把它们公开。
...
打包前请先跑本地门禁 `tmp/precheck.ps1`（所有 `.pm` 语法 + 设置页模板纯 ASCII + shim `node --check`）。
