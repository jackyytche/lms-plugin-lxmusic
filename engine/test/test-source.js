...
// 验证点：md5 正确性 / urlGet 纯 HTTPS / 3xx 手动跟随 / Promise 驱动 / RESULT 行协议
lx.on(lx.EVENT_NAMES.request, ({ action, info }) => {
	...
	if (action === 'redirect') {
		return new Promise((resolve, reject) => {
			...
			lx.request('http://github.com/', { timeout: 20 }, (err, body, status, headers) => {
				if (err) return reject(new Error('redirect err: ' + err));
				resolve({ status, len: String(body).length, ok200: status === 200 });
			});
		});
	}
