...
// 对齐 lx 宿主 needle follow_max:3）。脚本内部 Promise 由 qjs 顶层作业循环驱动。
// 注意：quickjs 无 TextEncoder/TextDecoder/URL/URLSearchParams，shim 手写等价物。
// ============================================================
...
// ---------- UTF-8（quickjs 无 TextEncoder/TextDecoder） ----------
function toUtf8(s) {
	s = String(s);
	const out = [];
...
if (args.length < 3) {
	print('RESULT ' + JSON.stringify({ ok: false, error: 'usage: qjs shim.mjs <source.js> <action> <infoJSON>' }));
	std.exit(1);
}
